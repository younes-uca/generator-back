...
String jwtToken = request.getHeader(${config.securityParams}.JWT_HEADER_NAME);
System.out.println("Token=" + jwtToken);
if (jwtToken == null || !jwtToken.startsWith(${config.securityParams}.HEADER_PREFIX)) {
filterChain.doFilter(request, response);
return;
}
JWTVerifier verifier = JWT.require(Algorithm.HMAC256(${config.securityParams}.SECRET)).build();
String jwt = jwtToken.substring(${config.securityParams}.HEADER_PREFIX.length());
