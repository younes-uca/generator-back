...
export class ${pojo.name}Service {
    ...
    public findByCriteria(${pojo.name?uncap_first}:${pojo.name}Vo){
        <#list pojo.fieldsSimpleNumberOrDate as field>
           <#if field.type.simpleName == "Date">
              ${pojo.name?uncap_first}.${field.name} = moment(${pojo.name?uncap_first}.${field.name}).format("YYYY-MM-DD");
           </#if>
        </#list>
