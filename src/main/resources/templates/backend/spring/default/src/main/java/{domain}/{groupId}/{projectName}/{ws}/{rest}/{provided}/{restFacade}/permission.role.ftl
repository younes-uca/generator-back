...
public class ${permissions[0].pojo.name?cap_first}Rest${roleName?cap_first} {
    ...
    @ApiOperation("Deletes a ${permissions[0].pojo.name?uncap_first} by ${permissions[0].pojo.id.name}")
    @DeleteMapping("/${permissions[0].pojo.id.name}/{${permissions[0].pojo.id.name}}")
    public int deleteBy${permissions[0].pojo.id.name?cap_first}(@PathVariable ${permissions[0].pojo.id.type.simpleName} ${permissions[0].pojo.id.name}){
        return ${permissions[0].pojo.name?uncap_first}Service.deleteBy${permissions[0].pojo.id.name?cap_first}(${permissions[0].pojo.id.name});
    }
    </#if>
    </#list>
}
