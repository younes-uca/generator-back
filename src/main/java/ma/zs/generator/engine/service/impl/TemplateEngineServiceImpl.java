...
public class TemplateEngineServiceImpl implements TemplateEngineService {
	...
	@Override
	public void scanTemplate(File template, String projectTemplatePath, List<Pojo> pojos, String outputFolder,
			ProjectConfig config, List<RoleConfig> roleConfigs) throws IOException, TemplateException {

		List<File> childs = Arrays.asList(template.listFiles());
		for (File file : childs) {
			if (file.isDirectory()) {

				// System.out.println("paaaath roooooooole name
				// *******************"+file.getName());
					...
					if (EngineUtil.isAuthorities(file.getName())) {
						...
						if (!EngineUtil.isTemplate(file.getName()))
							...
							if (EngineUtil.isTemplateForMany(file.getName())) {
								...
								generateOne(file, pojos, projectTemplatePath, outputFolder, config, roleConfigs);
...
//file parent = src\main\resources\templates\backend\spring\default\src\main\java\{domain}\{groupId}\{projectName}\{ws}\{rest}\{provided}\roles
			for (Pojo pojo : pojos) {
				freeMarkerService.generateFileWithOnePojo(pojo, file.getName(), file.getParent() + File.separator,
						pojo.getName() + suffix + "." + extensions, outputDirectory, config, roleConfigs);
			}
		} else {
			for (Pojo pojo : pojos) {
	...
	private void generateOptional(File file, List<Pojo> pojos, String src, String dest, ProjectConfig config)
		...
		if (isValidOptional(EngineUtil.getCondition(fileName), config)) {
			...
			freeMarkerService.generateFile(pojos, file.getName(), fileName, extension, outputDirectory,
					file.getParent() + File.separator, config, new ArrayList<RoleConfig>());
	...
	private void generateFilesInFolders(Pojo pojo, String templateName, String suffix, String extensions,
		...
		if (EngineUtil.inFolder(templateName)) {
			...
			freeMarkerService.generateFileWithOnePojo(pojo, templateName, templatePath,
					StringFormatterUtil.lowerCaseTheFirstLetter(pojo.getName()) + "-" + suffix + "." + extensions,
					outputDirectory, config, roleConfigs);
		} else {
			if (StringUtil.isEmpty(suffix))
				freeMarkerService.generateFileWithOnePojo(pojo, templateName, templatePath,
						StringFormatterUtil.lowerCaseTheFirstLetter(pojo.getName()) + "." + extensions, outputDirectory,
						config, roleConfigs);
			else
